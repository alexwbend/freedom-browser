...
It ships with integrated Swarm, IPFS, Radicle, and experimental Myotis nodes, enabling direct peer-to-peer network access without relying on centralized HTTP gateways. Radicle is available on macOS, Linux, and Windows x64/ARM64.
...
5. Swarm and IPFS nodes start automatically by default. Radicle and Myotis are first-class, opt-in embedded nodes under **Settings → Automatic Startup**. Enter a Swarm hash, IPFS CID, Radicle ID, `bzz://` URL, `ipfs://` URL, `rad://` URL, or `.eth`/`.box`/`.wei`/`.gwei`/`.tez` domain in the address bar.
...
| **Node Software**    | Ant (antd, bee-compatible) | freedom-ipfs native      | Myotis native addon            | libradicle native addon        | Arti SOCKS5 proxy             |
...
| **Managed Gateway Port** | 11633+     | internal native handler               | none; embedded native client   | internal native handler        | n/a                           |
| **Managed API Port** | 11633+         | internal native handler               | none; embedded native client   | internal native handler        | n/a                           |
| **Managed P2P Port** | 12633+         | internal native handler               | none; embedded native client   | in-process                     | n/a                           |
...
1. **Independent Managed Nodes**: Each profile has its own Ant, native IPFS, Myotis, Radicle, and Arti data directories. IPFS, Myotis, and Radicle use embedded native handlers without loopback API or gateway ports.
2. **Explicit External Nodes**: Profiles can opt into an external Swarm endpoint or Tor SOCKS5 endpoint. IPFS, Myotis, and Radicle always use their embedded native nodes.
...
- Run it standalone (bundled Swarm and native IPFS nodes start automatically; Radicle and Myotis startup are opt-in)
...
- **Profile Control**: Enable or disable Radicle for each profile under **Settings → Nodes**.
- **Node Toggle**: Start and stop Radicle from the Nodes panel; automatic startup remains opt-in.
...
- **Node Auto-start**: Toggle whether Swarm, IPFS, Radicle, and experimental Myotis start automatically at launch. Radicle and Myotis remain off by default.
...
- **Profile-Aware Radicle Errors**: Opening `rad://` while Radicle is disabled points to Settings > Nodes.
...
Named profiles use the next profile slot for Ant and Tor (`11634`, `19151`, and so on). The ecosystem default Swarm/Tor ports (`1633`, `9150`) are treated as external/system-node endpoints. IPFS, Myotis, and Radicle are native-only and do not expose or reuse loopback API/gateway ports.
...
- **Binary.** Arti has no clean prebuilt-binary distribution, so `npm run
  ...
  disabled until the binary is present. Tor is macOS/Linux-only for
...
- Ensure Radicle is managed for the profile under **Settings → Nodes**
...
- Ensure the respective node (Ant, IPFS, or Radicle) is running in the Nodes panel
