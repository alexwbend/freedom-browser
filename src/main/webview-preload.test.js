...
describe('webview-preload', () => {
  ...
  test('exposes guarded freedomAPI methods for allowed internal pages', async () => {
    ...
    const invokeCases = [
      ...
      ['getPayments', [{ kind: 'x402' }], IPC.PAYMENTS_GET_RECENT, [{ kind: 'x402' }]],
  ...
  test('routes package-hosted swarm publish requests to main trusted-prompt handling', async () => {
    const { ipcRenderer, postedMessages } = loadWebviewPreloadModule({
      location: {
        href: 'https://app.example/',
        protocol: 'https:',
  ...
  test('routes package-hosted swarm file publish requests to main trusted-prompt handling', async () => {
    const { ipcRenderer, postedMessages } = loadWebviewPreloadModule({
      location: {
        href: 'https://app.example/',
        protocol: 'https:',
    ...
    const params = {
      files: [{ path: 'index.html', bytes: { type: 'Buffer', data: [104, 111, 109, 101] } }],
  ...
  test('routes package-hosted swarm update-feed requests to main trusted-prompt handling', async () => {
    const { ipcRenderer, postedMessages } = loadWebviewPreloadModule({
      location: {
        href: 'https://app.example/',
        protocol: 'https:',
    ...
    const params = { feedId: 'blog', reference: 'aa'.repeat(32) };
    const messageHandlers = global.window.addEventListener.mock.calls
      .filter(([event]) => event === 'message')
      .map(([, handler]) => handler);
  ...
  test('routes package-hosted swarm chunk publishes to main trusted-prompt handling', async () => {
    const { ipcRenderer, postedMessages } = loadWebviewPreloadModule({
      location: {
        href: 'https://app.example/',
        protocol: 'https:',
