...
| `getSurfaceState`, `openSurface`, `closeSurface`, `toggleSurface` | unavailable through the renderer adapter while direct `freedomShell` methods existed | wallet/sidebar control could stay hidden or reach around the adapter instead of using the browser chrome API surface | now delegate to `freedomShell` surface-control methods gated by `surfaces.wallet.control`; official package smoke proves the visible wallet/sidebar button toggles shell-owned placeholder state without wallet/identity globals |
...
mode. The fixture package smoke exercises the same placeholder surface-control
path.
