...
`surfaces.swarmPublish.control`, and package-hosted feed update/write prompts
now show main-derived feed review context in the shell-owned approval window.
...
node/stamp/signer readiness; their shell-owned prompt details include
main-derived feed metadata and, for feed-entry writes, a bounded payload
preview. `swarm_getSigningIdentity` and
