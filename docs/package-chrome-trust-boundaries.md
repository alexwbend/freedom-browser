...
| Vault unlock | user or privileged flow | bundled wallet/identity UI and identity IPC | unavailable to package chrome | `trusted-surface` | shell-owned vault prompt | package-hosted x402 signing paths use a shell-owned trusted vault-unlock window with a dedicated preload; broader wallet/general unlock UX stays shell-owned | trusted prompt request cap, not identity raw API | broker unit/integration test plus x402 package-hosted retry coverage | implemented for package-hosted x402 sign/retry; broader/general unlock UX remains proposed deferral |
...
| x402 approvals | network intercept/provider | x402 intercept and bundled sidebar approval UI | adapter x402 methods/events were no-ops | `trusted-surface`, sometimes `provider-path` | shell-owned payment prompt | final approval and vault unlock in shell-owned prompts; package chrome may surface status only | trusted prompt or surface cap, not raw x402 IPC | package adapter unit coverage and launched package smoke for shell-owned prompt/no-host-event behavior | raw x402 methods return structured `X402_PACKAGE_API_UNAVAILABLE`, raw x402 host events are not delivered to package chrome, and package-hosted payment approvals can now sign/retry through a shell-owned native prompt; recognized EIP-155 token payments can also create the bounded default 10-token/30-day cap through that prompt, with official package smoke covering the cap option plus locked-vault fallback; if signing hits a locked vault, package-hosted x402 now opens a shell-owned trusted vault-unlock window and retries after successful unlock; cap editing/revocation APIs, payment permission management, payment history, and richer review surfaces still remain unavailable to package chrome |
...
receive raw x402 IPC, raw vault state, edit/revoke x402 caps, write arbitrary
payment permissions, or read payment history. File/folder/full publish UX,
richer account selection/review, x402 permission management, and broader
general vault unlock UX still need main-derived guest/request context and real
...
`swarm_publishFiles`, and `swarm_publishChunk` also use shell-owned prompt
paths: main derives the guest origin and package host identity, validates
data/files/chunk payloads before prompting, presents a native dialog, and
executes through the existing main-owned provider publish paths if the user
chooses Publish. `swarm_createFeed`
