...
| Identity onboarding | user in chrome/sidebar | onboarding and identity UI under bundled renderer with `window.identity` | package mode skips onboarding and lacks identity global | `trusted-surface`, `surface-control-api` | shell-owned identity surface | ordinary package chrome may request the shell-owned identity window; vault creation/import/unlock/lock remain shell-owned and scoped to the trusted window | `surfaces.identity.control` | unit coverage plus official package smoke for trusted-window open/state behavior | shell-owned trusted identity window implemented with dedicated preload and scoped IPC for create vault, import recovery phrase, unlock, and lock; package chrome still receives no raw identity, mnemonic, vault, or quick-unlock APIs |
| Vault unlock | user or privileged flow | bundled wallet/identity UI and identity IPC | unavailable to package chrome | `trusted-surface` | shell-owned vault prompt and identity surface | package-hosted x402 signing paths, package-hosted Ethereum signature/transaction paths, trusted wallet surface creation, and trusted identity onboarding use shell-owned trusted windows with dedicated preloads; broader non-provider vault settings stay shell-owned | trusted prompt request cap or `surfaces.identity.control`, not identity raw API | broker unit/integration test plus x402, wallet-provider package-hosted retry coverage, official package smoke for trusted wallet create unlock, and identity surface smoke | implemented for package-hosted x402 sign/retry, wallet-provider signature/transaction retry, trusted wallet derived-wallet creation, and trusted identity create/import/unlock/lock; broader non-provider vault management remains future work |
...
The identity window can create or import the recovery phrase vault and unlock
or lock it through scoped trusted-window IPC without exposing raw identity,
mnemonic, vault, or quick-unlock APIs to package chrome.
...
checkpoint still does not migrate broader non-provider vault management into
package mode.
...
Identity onboarding now opens a shell-owned trusted identity window from
package mode; broader non-provider vault management still needs real
shell-owned surfaces before it can be called complete in package mode.
