...
| Wallet sidebar button | toolbar button | `initSidebar` and `initWalletUi` run in bundled mode and use wallet/identity globals | package mode skipped sidebar/wallet init; button initially remained visible with no handler | `surface-control-api` | wallet surface is shell-owned | button requests and mirrors shell-owned surface state and must not initialize package-hosted wallet/identity UI | `surfaces.wallet.control` | official package smoke proving visible shell-owned trusted-window behavior and caller-scoped state-event updates; fixture smoke proving surface control | shell-owned trusted wallet window implemented and exercised by official package smoke; it displays public wallet accounts and dApp wallet permissions, can revoke dApp permissions, set the active wallet, create a derived wallet through the shell-owned vault unlock prompt when needed, rename wallets, delete non-main wallets after connected dApp grants are revoked, and export the vault seed phrase or a selected wallet private key after password verification through scoped trusted-window IPC |
...
| Transaction send/sign | website/dApp or wallet UI | wallet UI and wallet IPC under broad preload | unavailable to package chrome | `provider-path`, `trusted-surface` | shell-owned transaction/signing prompt | final approval rendered by shell-owned trusted surface; package chrome can only request surface/open state | none for package provider; surface caps only | trusted broker doc/tests and official package smoke before success migration | package-hosted `personal_sign` signs through a shell-owned trusted wallet approval window plus main/vault execution for connected origins; the window shows main-derived account choices, connected/requested account details, and bounded message review details; accepted selected-wallet indexes are revalidated in main, and switched dApp grants are persisted only after successful signing; if the vault is locked after approval, main opens the shell-owned trusted vault-unlock window and retries only after successful unlock; package-hosted `eth_sendTransaction` now sends through a shell-owned trusted wallet approval window plus main-side selected/requested account and chain validation, gas/fee preparation, vault access, trusted vault-unlock retry when needed, and the existing transaction recorder for connected origins; deprecated `eth_sign` still returns structured safe failures |
| Typed-data sign | website/dApp | wallet/dApp signing UI under bundled renderer | unavailable to package chrome | `provider-path`, `trusted-surface` | shell-owned signing prompt | same as transaction sign | none for package provider; surface caps only | trusted broker doc/tests before success migration | package-hosted `eth_signTypedData`, `eth_signTypedData_v3`, and `eth_signTypedData_v4` now sign through the shell-owned trusted wallet approval window plus main/vault execution for connected origins, including shell-owned vault-unlock retry when signing first finds the vault locked; the approval window shows main-derived account choices plus connected/requested account and bounded typed-data preview details, and main revalidates selected accounts before signing; unsupported typed-data variants still safe-fail |
| Identity onboarding | user in chrome/sidebar | onboarding and identity UI under bundled renderer with `window.identity` | package mode skips onboarding and lacks identity global | `trusted-surface`, `surface-control-api` | shell-owned identity surface | ordinary package chrome may request the shell-owned identity window; vault creation/import/unlock/lock/change/delete and quick-unlock controls remain shell-owned and scoped to the trusted window | `surfaces.identity.control` | unit coverage plus official package smoke for trusted-window open/state behavior | shell-owned trusted identity window implemented with dedicated preload and scoped IPC for create vault, import recovery phrase, unlock, lock, change password, typed-confirmation delete, and quick-unlock enable/disable; package chrome still receives no raw identity, mnemonic, vault, or quick-unlock APIs |
| Vault unlock | user or privileged flow | bundled wallet/identity UI and identity IPC | unavailable to package chrome | `trusted-surface` | shell-owned vault prompt and identity surface | package-hosted x402 signing paths, package-hosted Ethereum signature/transaction paths, trusted wallet surface creation, and trusted identity vault management use shell-owned trusted windows with dedicated preloads | trusted prompt request cap or `surfaces.identity.control`, not identity raw API | broker unit/integration test plus x402, wallet-provider package-hosted retry coverage, official package smoke for trusted wallet create unlock, and identity surface smoke | implemented for package-hosted x402 sign/retry, wallet-provider signature/transaction retry, trusted wallet derived-wallet creation, and trusted identity create/import/unlock/lock/change/delete/quick-unlock management |
...
| `getSurfaceState`, `openSurface`, `closeSurface`, `toggleSurface`, `onSurfaceStateChanged` | unavailable through the renderer adapter while direct `freedomShell` methods existed | wallet/sidebar control could stay hidden, reach around the adapter, or fail to mirror shell-owned state changes | now delegate to `freedomShell` surface-control methods; `wallet` is gated by `surfaces.wallet.control`, `identity` is gated by `surfaces.identity.control`, `payments` is gated by `surfaces.payments.control`, `swarmPublish` is gated by `surfaces.swarmPublish.control`, and `surfaces.stateChanged` derives its required capability from the event surface; official package smoke proves wallet and identity trusted-window behavior and the visible package-hosted payments and publish pages can open their shell-owned trusted windows |
...
payments window. The identity slice adds `surfaces.identity.control` for the
shell-owned trusted identity window. The Swarm publish slice adds
...
Wallet, identity, payments, and Swarm publish windows are bundled shell code with
...
wallet, create derived wallets through the shell-owned vault unlock prompt
when needed, rename wallets, delete non-main wallets after connected dApp
grants are revoked, and export the vault seed phrase or a selected wallet
private key after vault-password verification without exposing wallet,
identity, vault, mnemonic, private-key, or dApp permission-store APIs to
package chrome. Package-hosted provider signing and transaction prompts now
also show main-derived account choices and can switch the dApp grant only
after main revalidates the selected wallet and successfully signs or sends.
The identity window can create or import the recovery phrase vault, unlock or
lock it, change the vault password, delete the vault after typed confirmation,
and enable or disable quick unlock through scoped trusted-window IPC without
exposing raw identity, mnemonic, vault, or quick-unlock APIs to package chrome.
The payments window reads recent
...
checkpoint now keeps non-provider vault management in package mode behind the
shell-owned identity surface.
...
only the transaction hash. Signing and transaction prompts now include
main-derived account choices; accepted selections are revalidated in main, and
changed dApp wallet grants are persisted only after the selected-account
signature or transaction succeeds. Transactions with an explicit `from` still
fail unless that address matches the selected account. Locked-vault
transaction signing uses the same shell-owned unlock window and retry path.
This still does not expose vault primitives or raw wallet authority to package
chrome. The
...
index before prompting, shows display-safe feed name, payload size, current
reference, manifest reference, owner, feed identity, and bounded payload
preview where available, and writes through the existing main-owned provider
path after approval. These paths do not expose stamp management, enable the
full publish center, or expose raw Swarm IPC.
...
`surfaces.swarmPublish.control`, and package-hosted feed create/update/write
prompts now show a dedicated main-derived feed review section in the
shell-owned approval window.
Identity onboarding and non-provider vault management now open a shell-owned
trusted identity window from package mode.
...
signing, and persists a switched dApp wallet grant only after signing succeeds.
If the vault is locked, main opens the trusted vault-unlock window and retries
after successful unlock. `eth_sendTransaction` uses the shell-owned trusted
wallet approval window and sends in main through vault access and the existing
transaction recorder when the origin is already connected, the selected or
requested account and chain are valid, and the user chooses Send; the window
shows main-derived account choices plus account, chain, recipient, and value
preview. Transactions with an explicit `from` still fail unless that address
matches the selected account, and switched dApp grants are persisted only
after transaction success. Locked-vault execution uses the same trusted
unlock-and-retry path.
Deprecated `eth_sign` and unsupported signing variants return structured
provider errors in package mode. Other unsupported higher-risk methods return
`trusted_prompt_unavailable` before package chrome can broker them. Bundled
trusted chrome keeps the legacy renderer prompt path for those methods.
...
approval UI to package chrome. Wallet connect, package-hosted signing, and
package-hosted transactions now support selected-account grants through the
shell-owned prompt after main revalidation. Broader non-provider vault
management still requires additional shell-owned surface migration before
...
`shell_trusted_prompt_rejected` provider errors. Other unsupported higher-risk
methods return `trusted_prompt_unavailable` before package chrome can broker
them. Bundled trusted chrome keeps the legacy renderer prompt path for those
methods.
...
node/stamp/signer readiness; their shell-owned prompt details render as a
dedicated feed review with main-derived feed metadata and, for feed-entry
writes, a bounded payload preview. `swarm_getSigningIdentity` and
...
`guestContent.webviews: true`, and validates the result with
