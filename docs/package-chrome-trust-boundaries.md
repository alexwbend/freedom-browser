...
| History/autocomplete read | address bar UI | autocomplete/navigation use `electronAPI.getHistory` and main `src/main/history.js` | adapter returns `[]` | `browser-state-api` | none | expose history read if package autocomplete depends on it | `browserState.history.read` | harness-populated autocomplete smoke when implemented | required if visible autocomplete depends on it |
| History add | navigation lifecycle | renderer records visits with `electronAPI.addHistory` | adapter returns `false` | `browser-state-api` | none | expose add only if package mode records history from shell-owned navigation state | `browserState.history.write` | unit and smoke coverage for recording or explicit disabled behavior | conditional |
| Favicons | tabs, bookmarks, autocomplete | renderer calls favicon IPC in `src/main/favicons.js` | adapter returns `null` | `browser-state-api` | none | cached favicon read if visible package UI depends on icons | `browserState.favicons.read` | unit coverage and smoke if icons are asserted | conditional |
...
| Wallet connect | website provider request | page/provider code coordinates with renderer wallet UI and main permission stores | package chrome has no wallet globals; provider bypass not yet package-smoked | `provider-path`, then `trusted-surface` | shell-owned trusted prompt | guest content talks to main provider broker; package chrome does not broker or render final approval | provider capabilities are not package chrome caps | deterministic package provider-flow smoke | required provider safety proof; full wallet UX may be deferred with approval |
...
| `getHistory` | returns `[]` | autocomplete lacks history | delegate to `browserState.history.read` if package autocomplete uses history |
| `addHistory`, `removeHistory`, `clearHistory` | return `false` | history recording/management can silently fail | implement only if package UI exposes recording/management; otherwise hide/disable and test |
...
| `getFavicon`, `getCachedFavicon`, `fetchFavicon`, `fetchFaviconWithKey` | return `null` | tabs/bookmarks/autocomplete can lack icons | delegate cached read if visible UI depends on it; keep network fetch shell-owned |
